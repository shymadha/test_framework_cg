...
def discover_all_test_classes():
    ...
    Uses a robust filesystem walk to ensure all modules are found.
    """
    ...
    for root, _, files in os.walk(tests_dir):
        for f in files:
            if f.endswith(".py") and not f.startswith("__") and f != "base_test.py":
                # Calculate module path relative to framework.tests
                rel_path = os.path.relpath(root, tests_dir)
                if rel_path == ".":
                    mod_name = f"framework.tests.{f[:-3]}"
                else:
                    mod_name = f"framework.tests.{rel_path.replace(os.sep, '.')}.{f[:-3]}"

                try:
                    # Import module and find classes ending in 'Test'
